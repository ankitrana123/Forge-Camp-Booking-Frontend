...
export class Service {
  ...
  isAdmin(message: boolean) {
   ...
   /**
   * Get the average camp rating for all the rated camps 
   * @param rating rating given by the user to that camp
   */
  getCampRating(bookingReferenceNumber: string, rating: number) {
    return this.Http.get(
      this.baseUrl+`/Api/Camp/GetRatingForBookedCamps/${bookingReferenceNumber}/${rating}`
    );
  }
  ...
  getFilteredCamps(checkInDate: Date, CheckOutDate: Date, Capacity: number) {
    return this.Http.get(
      this.baseUrl +
        `/Api/Camp/GetCampsBetween/${checkInDate}/${CheckOutDate}/${Capacity}`
    );
  }
  /**
  ...
  deleteCamp(campId: string) {
    return this.Http.delete(this.baseUrl + `/Api/Camp/DeleteCamp/${campId}`, {
  ...
  /**
   * Create a booking for the given camp details
   */
  bookCamp(data) {
    return this.Http.post(this.baseUrl + '/Api/Booking/CreateBooking', data);
  }

  /**
   *authenticate the user using its userName and password
   */
